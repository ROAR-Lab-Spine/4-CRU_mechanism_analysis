...
def forward_kinematics(self):
    if self.operation_mode == "H1":
        ...
        if self.geometric_cond == "A":
            coeffs = fwd_kin.cal_tp_poly_coeffs_H3(self.joint_pos, self.a, self.b, self.c, self.d, self.r)
...
def tf_mat_from_tp(self, tp):
    if self.operation_mode == "H1":
        ...
        if self.geometric_cond == "A":
            Xsol, Ysol, Zsol = fwd_kin.cal_xyz_sol_H3(tp, self.joint_pos, self.a, self.b, self.c, self.d, self.r)
            x_1 = 2.0*tp/(1 + tp**2)
            x_0 = (1 - tp**2)/(1 + tp**2)
            t_mat = tfs.translation_matrix((Xsol, Ysol, Zsol))
